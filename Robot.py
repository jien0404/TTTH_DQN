...
class Robot:
    ...
    def get_state(self, obstacles, grid_width, grid_height, goal, vision_size=5):
        # Tính toán phạm vi dựa trên vision_size
        vision_range = vision_size // 2
        state = np.zeros((vision_size, vision_size)) # <-- SỬA Ở ĐÂY

        # Lặp dựa trên phạm vi đã tính
        for i in range(-vision_range, vision_range + 1):
            for j in range(-vision_range, vision_range + 1):
                ...
                # Cập nhật index cho mảng state
                state_y = i + vision_range
                state_x = j + vision_range

                # Kiểm tra biên
                if not (0 <= grid_x < grid_width and 0 <= grid_y < grid_height):
                    state[state_y, state_x] = -1  # Tường
                    continue
                ...
                for obs in obstacles:
                    ...
                    robot_check_pos = (
                        self.env_padding + (grid_x + 0.5) * self.cell_size,
                        self.env_padding + (grid_y + 0.5) * self.cell_size
                    )
                    if obstacle_rect.collidepoint(robot_check_pos):
                        state[state_y, state_x] = -1 # Vật cản
                        collided = True
                        break
                
                if not collided:
                    state[state_y, state_x] = 0 # Ô trống

        # Tính khoảng cách đến đích
        distance_to_goal = np.sqrt((self.grid_x - (goal[0] - self.env_padding) / self.cell_size)**2 +
                                (self.grid_y - (goal[1] - self.env_padding) / self.cell_size)**2)
